...
The default runtime workspace stays the host runtime folder, not this shared repo.
If a specific `Codex Agent` or `Codex CLI` run should use the shared repo itself as the workspace, set that node's `Working Directory` explicitly.
