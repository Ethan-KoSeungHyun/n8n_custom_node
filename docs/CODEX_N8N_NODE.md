...
- credential
  - `CodexChatgptAccount.credentials.js`
...
The Codex nodes use the host runtime folder as their default working directory because that is where n8n starts.
If a workflow should treat the shared repo clone as the workspace, set that node's `Working Directory` explicitly.
